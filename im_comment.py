...
class IMLinkToCommentBot:
	...
	if match:
		...
		comment += "<br />\n" + match.group(1).strip()
            ...
            tmpl_pattern = re.compile( ur'{{施設(.+?)}}',
                                       re.DOTALL )
            text = re.sub( tmpl_pattern,
                           ur'{{施設\n|備考=%s\1}}' % self.im_comment,
                           text )
...
def main():
    ...
    if gen:
        ...
        bot = IMLinkToCommentBot(gen, dry, always, input, outputwiki)
        bot.run()
    else:
        pywikibot.showHelp()
